...
router.get("/", async (req, res) => {
    ...
    res.sendFile("finalizar-compra.html", { root: "public" });
});

router.post("/", async (req, res) => {
    let session = (req.session) ? req.session : {};
    if (!session.user_id){ return res.redirect('/cadastro'); }
    ...
    if (!data){ return res.status(400).json({code: 1}); }
    if (!data.pagamento){ return res.status(400).json({code: 2}); }
    ...
    let result = await axios.post(request_url, proposta, { 
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${process.env.PORTO_API_TOKEN}` },
    });

    let novaProposta = {        
        numeroProposta: result.numeroProposta,
        numeroVersaoProposta: result.numeroVersaoProposta,
